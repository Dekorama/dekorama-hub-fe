...
function StepIndicator({ step, currentStep, onClickStep, disableStepIndicators }: StepIndicatorProps) {
  ...
  return (
    <motion.div onClick={handleClick} className="step-indicator" style={disableStepIndicators ? { pointerEvents: 'none', opacity: 0.5 } : {}} animate={status} initial={false}>
      <motion.div
        variants={{
          inactive: { scale: 1, backgroundColor: 'rgba(255,255,255,0.08)', color: 'rgba(255,255,255,0.5)' },
          active: { scale: 1, backgroundColor: '#ffffff', color: '#ffffff' },
          complete: { scale: 1, backgroundColor: '#ffffff', color: '#ffffff' }
        }}
...
function StepConnector({ isComplete }: StepConnectorProps) {
  const lineVariants: Variants = {
    ...
    complete: { width: '100%', backgroundColor: 'rgba(255,255,255,0.5)' }
  };
